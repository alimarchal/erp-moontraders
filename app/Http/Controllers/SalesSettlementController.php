...
class SalesSettlementController extends Controller
{
    ...
    public function store(StoreSalesSettlementRequest $request)
    {
        ...
        try {
            ...
            $settlement = SalesSettlement::create([
                ...
                'credit_sales_data' => !empty($request->credit_sales) ? json_encode($request->credit_sales) : null,
                ...
                // Note: expenses, bank_transfers, cheques now stored in normalized tables
                'status' => 'draft',
                'notes' => $request->notes,
            ]);
    ...
    public function update(UpdateSalesSettlementRequest $request, SalesSettlement $salesSettlement)
    {
        ...
        try {
            ...
            // Store draft credit sales data (only processed when settlement is posted)
            // Note: CustomerCreditSale records and CustomerLedger entries are created
            // via LedgerService when the settlement is posted, not in draft state
            // This maintains proper transaction integrity and draft/posted workflow
