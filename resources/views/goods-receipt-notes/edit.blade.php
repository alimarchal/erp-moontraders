<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right">
            <a href="{{ route('goods-receipt-notes.show', $grn) }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('goods-receipt-notes.update', $grn) }}" id="grnForm"
                        x-data="grnForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                            <div>
                                ...
                                <x-input id="receipt_date" name="receipt_date" type="date" class="mt-1 block w-full"
                                    :value="old('receipt_date', $grn->receipt_date?->format('Y-m-d'))" required />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="supplier_id" name="supplier_id" required
                                    ...
                                    @foreach ($suppliers as $supplier)
                                                                    <option value="{{ $supplier->id }}" {{ old('supplier_id', $grn->
                            ...
                            <div>
                                ...
                                <select id="warehouse_id" name="warehouse_id" required
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                                                    <option value="{{ $warehouse->id }}" {{ old('warehouse_id', $grn->
                            ...
                            <div>
                                ...
                                <x-input id="supplier_invoice_date" name="supplier_invoice_date" type="date"
                                    class="mt-1 block w-full" :value="old('supplier_invoice_date', $grn->supplier_invoice_date?->format('Y-m-d'))" />
                            </div>
                            <div>
                                ...
                                <x-input id="supplier_invoice_number" name="supplier_invoice_number" type="text"
                                    class="mt-1 block w-full" :value="old('supplier_invoice_number', $grn->supplier_invoice_number)" />
                            </div>
    ...
    @push('scripts')
        <script>
            ...
            const existingItems = @json($grn->items);
            const defaultUomId = 24; // Piece UOM
            const defaultPurchaseUomId = 34; // Case UOM
            ...
            function grnForm() {
                return {
                    ...
                    items: oldItems.length > 0 ? oldItems.map(item => ({
                        ...
                        qty_in_stock_uom: parseFloat(item.qty_in_stock_uom) || 0,
                        unit_price_per_case: parseFloat(item.unit_price_per_case) || 0,
                        extended_value: parseFloat(item.extended_value) || 0,
                        discount_value: parseFloat(item.discount_value) || 0,
                        fmr_allowance: parseFloat(item.fmr_allowance) || 0,
                        discounted_value_before_tax: parseFloat(item.discounted_value_before_tax) || 0,
                        excise_duty: parseFloat(item.excise_duty) || 0,
            ...
            function initializeGRNForm() {
                ...
                $(document).ready(function () {
                    ...
                    // If there's supplier value (from old input or existing GRN), trigger product refresh
