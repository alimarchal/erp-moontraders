...
The dev container automatically configures the following database settings:
...
1. Wait for MySQL to fully start (takes 30-60 seconds)
...
# Change DB_USERNAME to "moontrader"
# Change DB_PASSWORD to "secret"
...
```dockerfile
RUN docker-php-ext-install pdo_mysql mbstring exif pcntl bcmath gd zip your-extension
```
...
To use PostgreSQL instead of MySQL, edit `docker-compose.yml`:

1. Replace `mysql` service with `postgres`
...
Database data is stored in a named volume `mysql-data`, which persists even if you rebuild the container. To reset the database:
