<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight inline-block">
            Edit Product Recall - {{ $recall->recall_number }}
        </h2>
        <div class="flex justify-center items-center float-right">
            <a href="{{ route('product-recalls.show', $recall) }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('product-recalls.update', $recall) }}"
                        id="productRecallEditForm" x-data="productRecallEditForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                            <div>
                                ...
                                <x-input id="recall_date" name="recall_date" type="date" class="mt-1 block w-full"
                                    :value="old('recall_date', $recall->recall_date->format('Y-m-d'))" required />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="supplier_id" name="supplier_id" required x-model="supplierId"
                                    ...
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ $recall->supplier_id == $supplier->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="warehouse_id" name="warehouse_id" required x-model="warehouseId"
                                    ...
                                    @foreach ($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" {{ $recall->warehouse_id == $warehouse->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="recall_type" name="recall_type" required
                                    ...
                                    @foreach (['supplier_initiated', 'quality_issue', 'expiry', 'other'] as $type)
                                        <option value="{{ $type }}" {{ $recall->recall_type == $type ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 gap-4 mb-6">
                            <div>
                                ...
                                <textarea id="reason" name="reason" rows="2" required
                                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('reason', $recall->reason) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6"
                            ...
                            <h4 class="text-sm font-semibold text-blue-800 mb-3">Search Additional Batches</h4>
                            ...
                            <div x-show="availableBatches.length > 0" class="mt-4">
                                <p class="text-xs text-blue-600 mb-2"
                                    x-text="`${availableBatches.length} batch(es) found:`"></p>
                                    ...
                                    <table class="w-full text-sm">
                                        <thead class="bg-blue-100 sticky top-0">
                                            <tr>
                                                ...
                                                <th class="px-2 py-1 text-right text-xs">Stock</th>
                                                <th class="px-2 py-1 text-right text-xs">Cost</th>
        ...
        ['label' => 'Action', 'align' => 'text-center', 'width' => '80px'],
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr x-show="items.length === 0">
                                    <td colspan="8" class="px-4 py-6 text-center text-sm text-gray-500">
                                        No items. Use the batch search above to add batches.
                                    </td>
                                </tr>
                            </tbody>
                            <tfoot class="bg-gray-50" x-show="items.length > 0">
                                <tr class="font-semibold bg-gray-100">
                                    ...
                                    <td class="px-2 py-2 text-right"
                                        x-text="formatNumber(items.reduce((s, i) => s + (parseFloat(i.quantity_recalled) || 0), 0))">
                                    </td>
                                    <td class="px-2 py-2"></td>
                                    <td class="px-2 py-2 text-right font-bold text-lg text-red-600"
                                        x-text="formatNumber(items.reduce((s, i) => s + (parseFloat(i.total_value) || 0), 0))">
                                    </td>
                                    <td class="px-2 py-2"></td>
                                </tr>
                            </tfoot>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="notes" name="notes" rows="3"
                                class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('notes', $recall->notes) }}</textarea>
                        </div>

                        <div class="flex items-center justify-end mt-6">
                            <x-button>Update Recall</x-button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            function productRecallEditForm() {
                return {
                    supplierId: '{{ old("supplier_id", $recall->supplier_id) }}',
                    warehouseId: '{{ old("warehouse_id", $recall->warehouse_id) }}',
                    items: @json($recall->items->map(fn($item) => [
                        'product_id' => (string) $item->product_id,
                        'product_name' => $item->product?->product_name ?? 'Unknown',
                        'stock_batch_id' => (string) $item->stock_batch_id,
                        'batch_code' => $item->stockBatch?->batch_code ?? 'N/A',
                        'available_qty' => (float) $item->quantity_recalled,
                        'quantity_recalled' => (float) $item->quantity_recalled,
                        'unit_cost' => (float) $item->unit_cost,
                        'total_value' => (float) $item->total_value,
                    ])->values()),
                    availableBatches: [],
                    searchLoading: false,
                    searchFilters: { batch_code: '', expiry_from: '', expiry_to: '' },

                    onSupplierChange() {
                        this.availableBatches = [];
                        if (this.supplierId && this.warehouseId) { this.searchBatches(); }
                    },
                        ...
                        try {
                            ...
                            this.availableBatches = batches.map(batch => ({
                                ...
                                stock_qty: batch.current_stock_by_batch
                                    ? batch.current_stock_by_batch.reduce((sum, s) => sum + parseFloat(s.quantity_on_hand || 0), 0) : 0,
                            }));
                        } catch (error) {
                    ...
                    calculateRecallValue(index) {
                        ...
                        item.total_value = (parseFloat(item.quantity_recalled) || 0) * (parseFloat(item.unit_cost) || 0);
                    },
