        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select id="filter_has_promotional" name="filter[has_promotional]"
                    ...
                    <option value="1" {{ request('filter.has_promotional') == '1' ? 'selected' : '' }}>Promotional Only</option>
                </select>
            </div>
        </div>
    </x-filter-section>
                ...
                <table class="report-table">
                    ...
                    <tbody>
                        @forelse ($stocks as $stock)
                            <tr>
                                <td class="text-center">{{ $loop->iteration + ($stocks->currentPage() - 1) * $stocks->perPage() }}</td>
                                <td class="text-left font-semibold px-2 whitespace-nowrap">{{ $stock->product->product_name }}</td>
                                <td class="text-left px-2 whitespace-nowrap">{{ $stock->warehouse->warehouse_name }}</td>
                                <td class="text-center font-semibold">{{ rtrim(rtrim(number_format($stock->quantity_on_hand, 2), '0'), '.') }}</td>
                                <td class="text-center">{{ rtrim(rtrim(number_format($stock->quantity_available, 2), '0'), '.') }}</td>
                                ...
                                <td class="text-center">
                                    ...
                                    @if($stock->promotional_batches > 0)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 ml-1">
                    ...
                    <tfoot>
                        <tr class="bg-gray-100 font-bold border-t-2 border-black">
                            ...
                            <td class="text-center">{{ rtrim(rtrim(number_format($stocks->sum('quantity_on_hand'), 2), '0'), '.') }}</td>
                            <td class="text-center">{{ rtrim(rtrim(number_format($stocks->sum('quantity_available'), 2), '0'), '.') }}</td>
