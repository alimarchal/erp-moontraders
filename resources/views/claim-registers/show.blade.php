@php
    $statusOptions = \App\Models\ClaimRegister::statusOptions();
    $transactionTypeOptions = \App\Models\ClaimRegister::transactionTypeOptions();
    $statusLabel = $statusOptions[$claimRegister->status] ?? ucfirst($claimRegister->status);
    ...
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right gap-2">
            @if (! $isPosted)
                ...
                <form id="postClaimForm" action="{{ route('claim-registers.post', $claimRegister->id) }}"
                    method="POST" onsubmit="return confirmPostClaim();" class="inline-block">
                    @csrf
                    ...
                    @if ($isPosted)
                        <div class="mb-4 flex items-center gap-2">
                            ...
                            @if ($claimRegister->journalEntry)
                                <span class="text-sm text-gray-500">
                                    &mdash; JE #{{ $claimRegister->journalEntry->entry_number ?? $claimRegister->journal_entry_id }}
                                </span>
                            @endif
                        </div>
                    @else
                    ...
                    <form>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                            ...
                            <div>
                                ...
                                <x-input id="claim_month" type="text" class="mt-1 block w-full cursor-not-allowed bg-gray-100"
                                    :value="$claimRegister->claim_month ?? '-'" disabled readonly />
                            </div>
                            <div>
                                <x-label for="date_of_dispatch" value="Date of Dispatch" />
                                <x-input id="date_of_dispatch" type="text" class="mt-1 block w-full cursor-not-allowed bg-gray-100"
                                    :value="$claimRegister->date_of_dispatch?->format('d-m-Y') ?? '-'" disabled readonly />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 mt-4">
                            <div>
                                ...
                                <textarea id="notes" rows="2" class="mt-1 block w-full cursor-not-allowed bg-gray-100 border-gray-300 rounded-md"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                <x-input id="created_at" type="text" class="mt-1 block w-full cursor-not-allowed bg-gray-100"
                                    :value="$claimRegister->created_at?->format('d-m-Y H:i:s') ?? '-'" disabled readonly />
                            </div>
                            <div>
                                ...
                                <x-input id="updated_at" type="text" class="mt-1 block w-full cursor-not-allowed bg-gray-100"
                                    :value="$claimRegister->updated_at?->format('d-m-Y H:i:s') ?? '-'" disabled readonly />
                            </div>
                        </div>
                    </form>
                </div>
