<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Customers" :createRoute="route('customers.create')" createLabel="Add Customer"
            createPermission="customer-create" :showSearch="true" :showRefresh="true" backRoute="settings.index" />
    </x-slot>

    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-2 no-print">
        <div class="flex justify-end">
            <a href="{{ route('customers.export.excel', request()->query()) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                ...
                <x-input id="filter_customer_name" name="filter[customer_name]" type="text" class="mt-1 block w-full"
                    :value="request('filter.customer_name')" placeholder="Search by name" />
            </div>

            <div>
                <x-label for="filter_customer_code" value="Customer Code" />
                <x-input id="filter_customer_code" name="filter[customer_code]" type="text"
                    class="mt-1 block w-full uppercase" :value="request('filter.customer_code')"
                    placeholder="CUST-001" />
            </div>

            <div>
                <x-label for="filter_city" value="City" />
                <x-input id="filter_city" name="filter[city]" type="text" class="mt-1 block w-full"
                    :value="request('filter.city')" placeholder="Muzaffarabad" />
            </div>

            <div>
                <x-label for="filter_sub_locality" value="Area / Sub Locality" />
                <x-input id="filter_sub_locality" name="filter[sub_locality]" type="text" class="mt-1 block w-full"
                    :value="request('filter.sub_locality')" placeholder="Satellite Town" />
            </div>
            ...
            <div>
                ...
                <select id="sort" name="sort"
                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                    <option value="customer_name" {{ request('sort', 'customer_name') === 'customer_name' ? 'selected' : '' }}>Name (A-Z)</option>
                    <option value="-customer_name" {{ request('sort') === '-customer_name' ? 'selected' : '' }}>Name (Z-A)</option>
                    <option value="customer_code" {{ request('sort') === 'customer_code' ? 'selected' : '' }}>Code (A-Z)</option>
                    <option value="-customer_code" {{ request('sort') === '-customer_code' ? 'selected' : '' }}>Code (Z-A)</option>
                    <option value="-credit_limit" {{ request('sort') === '-credit_limit' ? 'selected' : '' }}>Credit Limit (High-Low)</option>
                    <option value="credit_limit" {{ request('sort') === 'credit_limit' ? 'selected' : '' }}>Credit Limit (Low-High)</option>
                    <option value="city" {{ request('sort') === 'city' ? 'selected' : '' }}>City (A-Z)</option>
                </select>
            </div>
    ...
    <x-data-table :items="$customers" :headers="[
        ...
        :emptyRoute="route('customers.create')" emptyLinkText="Add a customer">
