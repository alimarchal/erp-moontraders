<x-app-layout>
    <x-slot name="header">
        ...
        <x-page-header title="Vehicles" :createRoute="route('vehicles.create')" createLabel="Add Vehicle"
            createPermission="vehicle-create" :showSearch="true" :showRefresh="true" backRoute="settings.index" />
        {{--
        <div class="ml-4">
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-2 no-print">
        <div class="flex justify-end">
            <a href="{{ route('vehicles.export.excel', request()->query()) }}"
    ...
    <x-filter-section :action="route('vehicles.index')">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <x-label for="filter_make_model" value="Make / Model" />
                <x-input id="filter_make_model" name="filter[make_model]" type="text" class="mt-1 block w-full"
                    :value="request('filter.make_model')" placeholder="Toyota Hilux" />
            </div>

            <div>
                <x-label for="filter_year" value="Year" />
                <x-input id="filter_year" name="filter[year]" type="number" class="mt-1 block w-full"
                    :value="request('filter.year')" placeholder="2024" />
            </div>
            ...
            <div>
                ...
                <select id="sort" name="sort"
                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                    <option value="vehicle_number" {{ request('sort') === 'vehicle_number' ? 'selected' : '' }}>Vehicle # (A-Z)</option>
                    <option value="-vehicle_number" {{ request('sort') === '-vehicle_number' ? 'selected' : '' }}>Vehicle # (Z-A)</option>
                    <option value="registration_number" {{ request('sort') === 'registration_number' ? 'selected' : '' }}>Registration (A-Z)</option>
                    <option value="vehicle_type" {{ request('sort') === 'vehicle_type' ? 'selected' : '' }}>Type (A-Z)</option>
                    <option value="-year" {{ request('sort') === '-year' ? 'selected' : '' }}>Year (Newest)</option>
                    <option value="year" {{ request('sort') === 'year' ? 'selected' : '' }}>Year (Oldest)</option>
                </select>
            </div>
    ...
    ]" emptyMessage="No vehicles found." :emptyRoute="route('vehicles.create')"
        emptyLinkText="Add a vehicle">
