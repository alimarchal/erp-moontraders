<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Product Tax Mappings" :createRoute="route('product-tax-mappings.create')"
            createLabel="Add Mapping" createPermission="tax-manage-mapping" :showSearch="true" :showRefresh="true"
            backRoute="settings.index" />
    </x-slot>

    <x-filter-section :action="route('product-tax-mappings.index')">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select id="filter_transaction_type" name="filter[transaction_type]"
                    ...
                    <option value="sales" {{ request('filter.transaction_type') === 'sales' ? 'selected' : '' }}>Sales
                    </option>
                    <option value="purchase" {{ request('filter.transaction_type') === 'purchase' ? 'selected' : '' }}>
                        Purchase</option>
                    <option value="both" {{ request('filter.transaction_type') === 'both' ? 'selected' : '' }}>Both
                    </option>
                </select>
            </div>
    ...
    <x-data-table :items="$mappings" :headers="[
        ['label' => '#', 'align' => 'text-center'],
        ['label' => 'Product'],
        ['label' => 'Tax Code'],
        ['label' => 'Transaction Type', 'align' => 'text-center'],
        ['label' => 'Status', 'align' => 'text-center'],
        ['label' => 'Actions', 'align' => 'text-center'],
    ]" emptyMessage="No product tax mappings found."
        :emptyRoute="route('product-tax-mappings.create')" emptyLinkText="Add a mapping">
        @foreach ($mappings as $index => $mapping)
            <tr class="border-b border-gray-200 text-sm">
                ...
                <td class="py-1 px-2 font-semibold">
                    {{ $mapping->product->product_code ?? 'N/A' }} - {{ $mapping->product->product_name ?? 'N/A' }}
                </td>
                <td class="py-1 px-2">
                    {{ $mapping->taxCode->tax_code ?? 'N/A' }}
                </td>
                <td class="py-1 px-2 text-center">
                    <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full
                            @class([
                                'bg-blue-100 text-blue-700' => $mapping->transaction_type === 'sales',
                                'bg-green-100 text-green-700' => $mapping->transaction_type === 'purchase',
                                'bg-purple-100 text-purple-700' => $mapping->transaction_type === 'both',
                            ])">
                        {{ ucfirst($mapping->transaction_type) }}
                    </span>
                </td>
                <td class="py-1 px-2 text-center">
                    <span
                        class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full {{ $mapping->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600' }}">
                        {{ $mapping->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </td>
                <td class="py-1 px-2 text-center">
                    <div class="flex justify-center space-x-2">
                        <a href="{{ route('product-tax-mappings.show', $mapping) }}"
                        ...
                        @endcan
                        @role('super-admin')
                        <form method="POST" action="{{ route('product-tax-mappings.destroy', $mapping) }}"
