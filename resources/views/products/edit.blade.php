    ...
    @push('scripts')
        <script>
            ...
            function productEditForm() {
                const original = {
                    product_code:           {{ Js::from($product->product_code) }},
                    product_name:           {{ Js::from($product->product_name) }},
                    description:            {{ Js::from($product->description ?? '') }},
                    supplier_id:            {{ Js::from((string) ($product->supplier_id ?? '')) }},
                    category_id:            {{ Js::from((string) ($product->category_id ?? '')) }},
                    valuation_method:       {{ Js::from($product->valuation_method) }},
                    uom_id:                 {{ Js::from((string) $product->uom_id) }},
                    sales_uom_id:           {{ Js::from((string) ($product->sales_uom_id ?? '')) }},
                    uom_conversion_factor:  {{ Js::from((string) $product->uom_conversion_factor) }},
                    brand:                  {{ Js::from($product->brand ?? '') }},
                    barcode:                {{ Js::from($product->barcode ?? '') }},
                    pack_size:              {{ Js::from($product->pack_size ?? '') }},
                    weight:                 {{ Js::from((string) ($product->weight ?? '')) }},
                    cost_price:             {{ Js::from((string) ($product->cost_price ?? '')) }},
                    unit_sell_price:        {{ Js::from((string) ($product->unit_sell_price ?? '')) }},
                    expiry_price:           {{ Js::from((string) ($product->expiry_price ?? '')) }},
                    reorder_level:          {{ Js::from((string) ($product->reorder_level ?? '')) }},
