<x-app-layout>
    <x-slot name="header">
        <x-page-header title="General Ledger"
            :createRoute="null"
            createLabel=""
            :showSearch="true"
            :showRefresh="true"
    ...
    <x-filter-section :action="route('reports.general-ledger.index')">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <x-input id="filter_entry_date_to" name="filter[entry_date_to]" type="date"
                    class="mt-1 block w-full" :value="request('filter.entry_date_to')" />
            </div>

            <div>
                <x-label for="filter_account_code" value="Account Code" />
                <x-input id="filter_account_code" name="filter[account_code]" type="text"
                    class="mt-1 block w-full uppercase" :value="request('filter.account_code')" placeholder="e.g., 4000" />
            </div>

            <div>
                <x-label for="filter_account_name" value="Account Name" />
                <x-input id="filter_account_name" name="filter[account_name]" type="text"
                    class="mt-1 block w-full" :value="request('filter.account_name')" placeholder="Search by account name" />
            </div>
            ...
            <div>
                ...
                <x-input id="filter_line_description" name="filter[line_description]" type="text"
                    class="mt-1 block w-full" :value="request('filter.line_description')" placeholder="Line description contains..." />
            </div>
            ...
            <div>
                ...
                <x-input id="filter_cost_center_code" name="filter[cost_center_code]" type="text"
                    class="mt-1 block w-full uppercase" :value="request('filter.cost_center_code')" placeholder="e.g., CC-01" />
            </div>

            <div>
                <x-label for="filter_currency_code" value="Currency" />
                <x-input id="filter_currency_code" name="filter[currency_code]" type="text"
                    class="mt-1 block w-full uppercase" :value="request('filter.currency_code')" placeholder="e.g., USD" />
            </div>
