                                ...
                                <div class="bg-white rounded-lg border border-orange-300 overflow-hidden">
                                    ...
                                    <div class="p-0">
                                        ...
                                        <input type="hidden" id="total_bank_transfers" name="total_bank_transfers"
                                            value="0.00" />
                                        <input type="hidden" id="bank_transfers" name="bank_transfers" value="[]" />
                                        <input type="hidden" id="total_bank_slips" name="total_bank_slips"
                                            value="0.00" />
                                        <input type="hidden" id="bank_slips" name="bank_slips" value="[]" />
                                                    ...
                                                    <tr class="hover:bg-gray-50">
                                                        ...
                                                        <td class="py-1 px-1 text-right">
                                                            <input type="number" :id="'expense_amount_' + expense.id"
                                                                ...
                                                                :class="['1161', '5252', '5262', '5223'].includes(expense.account_code) ? 'bg-gray-100' : ''"
    ...
    @push('scripts')
        <script>
            ...
            function expenseManager(expenseAccounts) {
                return {
                    ...
                    onAccountSelect(event, index) {
                        ...
                        if (selectedId) {
                            const account = this.availableAccounts.find(acc => acc.id === selectedId);
                            if (account) {
                                this.expenses[index].label = account.account_name;
                                this.expenses[index].account_code = account.account_code; // Dynamically update account code
                                this.expenses[index].expense_account_id = selectedId;
                            }
                        } else {
                            this.expenses[index].label = 'New Expense';
                            this.expenses[index].account_code = '';
                            this.expenses[index].expense_account_id = '';
                        }
                    },
            ...
            $(document).ready(function () {
                ...
                @if(old('goods_issue_id'))
                    ...
                    fetch('{{ url('api/sales-settlements/goods-issues') }}/' + oldGoodsIssueId + '/items')
                        ...
                        .then(data => {
                            // Restore supplier selection first
                            if (data.supplier_id) {
                                $('#supplier_id').val(data.supplier_id).trigger('change.select2');
                                $('#goods_issue_id').prop('disabled', false);
                            }
            ...
            function updateSalesSummary() {
                ...
                const shortExcess = (actualPhysicalCash + chequeSalesAmount + bankSlipsAmount) - expectedCashNet;
                ...
                if (cashReceivedDisplay) cashReceivedDisplay.textContent = formatPKR(actualPhysicalCash + bankSlipsAmount); // Include Bank Slips here
                if (shortExcessDisplay) shortExcessDisplay.textContent = formatPKR(shortExcess);

                // Update summary_cash_received hidden input
                document.getElementById('summary_cash_received').value = (actualPhysicalCash + bankSlipsAmount).toFixed(2); // Include Bank Slips here
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        <td class="py-1 px-1 text-right">
                                                                                                                                                                                                                            <input type="number"
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                min="0"
                                                                                                                                                                                                                                step="1"
                                                                                                                                                                                                                                value="${Math.round(parseFloat(batch.quantity))}"
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        <td class="py-1 px-1 text-right">
                                                                                                                                                                                                                            <span id="bf-out-${index}-${batchIdx}" class="${batchBfQuantity === 0 ? 'font-bold text-green-600' : 'font-bold text-orange-600'}">${batchBfQuantity}</span>
                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                    </tr>
                                                                                                                                                                                                                `;
                    ...
                    updateDisplay() {
                        ...
                        if (this.entries.length === 0) {
                            ...
                            this.entries.forEach((entry, index) => {
                                ...
                                row.innerHTML = `
                                                                                        <td class="py-1 px-2 text-xs text-gray-800">${entry.bank_account_name || 'Unknown Account'}</td>
                                                                                        <td class="py-1 px-2 text-xs text-gray-600">
                                                                                            <div>${entry.deposit_date || 'N/A'}</div>
                                                                                        </td>
                                                                                        <td class="py-1 px-2 text-xs text-gray-700">${entry.reference_number || 'No ref'}</td>
                        ...
                        if (totalDisplay) {
                            totalDisplay.textContent = '₨ ' + total.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
