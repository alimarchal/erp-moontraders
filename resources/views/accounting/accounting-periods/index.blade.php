            ...
            <td class="py-1 px-2 text-center">
                <div class="flex justify-center space-x-2">
                    ...
                    {{-- <form method="POST" action="{{ route('accounting-periods.destroy', $period) }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="inline-flex items-center justify-center w-8 h-8 text-red-600 hover:text-red-800 hover:bg-red-100 rounded-md transition-colors duration-150 {{ $deleteDisabled ? 'opacity-40 cursor-not-allowed hover:bg-transparent hover:text-red-600' : '' }}"
                            title="Delete" {{ $deleteDisabled ? 'disabled aria-disabled=true' : '' }}>
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
