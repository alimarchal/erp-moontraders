<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Companies" :createRoute="route('companies.create')" createLabel="Add Company"
            createPermission="company-create" :showSearch="true" :showRefresh="true" backRoute="settings.index" />
    </x-slot>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <x-input id="filter_company_name" name="filter[company_name]" type="text" class="mt-1 block w-full"
                    :value="request('filter.company_name')" placeholder="Moon Traders" />
            </div>

            <div>
                <x-label for="filter_abbr" value="Abbreviation" />
                <x-input id="filter_abbr" name="filter[abbr]" type="text" class="mt-1 block w-full"
                    :value="request('filter.abbr')" placeholder="MT" />
            </div>
    ...
    <x-data-table :items="$companies" :headers="[
        ...
        :emptyRoute="route('companies.create')" emptyLinkText="Add a company">
