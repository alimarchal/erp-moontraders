    ...
    <x-filter-section :action="route('currencies.index')">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <x-input id="filter_currency_name" name="filter[currency_name]" type="text"
                    class="mt-1 block w-full" :value="request('filter.currency_name')" placeholder="Search by name" />
            </div>
    ...
    <x-data-table :items="$currencies" :headers="[
        ...
        @foreach ($currencies as $index => $currency)
            <tr class="border-b border-gray-200 dark:border-gray-700 text-sm">
                ...
                <td class="py-1 px-2 text-center">
                    <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full {{ $currency->is_base_currency ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-600' }}">
                        {{ $currency->is_base_currency ? 'Base' : 'Secondary' }}
                    </span>
                </td>
                <td class="py-1 px-2 text-center">
                    <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full {{ $currency->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
