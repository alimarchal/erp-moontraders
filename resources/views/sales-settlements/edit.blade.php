<x-app-layout>
    ...
    @php
        ...
        $creditSalesData = $settlement->customerEmployeeTransactions
            ->where('transaction_type', 'credit_sale')
            ->map(function ($sale) {
                return [
                    'customer_id' => $sale->account->customer_id,
                    'customer_name' => $sale->account->customer->customer_name ?? 'Unknown',
                    'invoice_number' => $sale->invoice_number,
                    'sale_amount' => $sale->debit,
                    'payment_received' => $sale->credit,
                    'notes' => $sale->notes
                ];
            });
