<x-app-layout>
    ...
    @php
        ...
        foreach ($settlement->items as $item) {
            foreach ($item->batches as $batch) {
                $savedBatchesData[$item->goods_issue_item_id . '_' . $batch->stock_batch_id] = [
        ...
        $creditSalesData = $settlement->creditSales->map(function ($sale) {
            return [
                ...
                'previous_balance' => (float) $sale->previous_balance,
                'new_balance' => (float) $sale->new_balance,
                'notes' => $sale->notes,
            ];
        });
        ...
        $chequesData = $settlement->cheques->map(function ($cheque) {
            return [
                ...
                'bank_account_name' => $cheque->bankAccount->account_name ?? null,
                'cheque_date' => optional($cheque->cheque_date)->format('Y-m-d'),
                'customer_id' => $cheque->customer_id,
                'customer_name' => $cheque->customer->customer_name ?? 'N/A',
                'notes' => $cheque->notes,
            ];
        });
    ...
    <div class="py-6">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form method="POST" action="{{ route('sales-settlements.update', $settlement) }}" id="settlementForm">
                    ...
                    <div class="pt-6 pl-6 pr-6">
                        ...
                        <p class="text-sm text-gray-500 " id="noItemsMessage" style="display: none;">
                    ...
                    <div class="pr-6 pl-6 pt-2">
                        ...
                        <div id="expenseAndSalesSummarySection" style="display: none;" class="mb-4">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6 items-start">
                                        ...
                                        <table class="w-full text-xs">
                                            <thead>
                                                <tr class="border-b-2 border-gray-300">
                                                    ...
                                                    <th class="py-1 px-1 text-right text-black">PB</th>
                                ...
                                {{-- Bank Slips Card --}}
                                <div class="bg-white rounded-lg border border-purple-300 overflow-hidden"
                                    x-data="bankSlipsDisplay()">
                                    <div
                                        class="bg-gradient-to-r from-purple-500 to-purple-600 px-3 py-2 flex justify-between items-center">
                                        <h4 class="text-sm font-bold text-white">Bank Slips / Deposits</h4>
                                        <button type="button" @click="openModal()"
                                            class="text-xs bg-white text-purple-600 px-2 py-0.5 rounded font-semibold hover:bg-purple-50">
                                            + Add
                                        </button>
                                        ...
                                        <table class="w-full text-xs">
                                            <thead>
                                                <tr class="border-b-2 border-gray-300">
                                                    <th class="py-1 px-1 text-left text-black">Bank</th>
                                                    <th class="py-1 px-1 text-left text-black">Ref #</th>
                                                    <th class="py-1 px-1 text-left text-black">Date</th>
                                                    <th class="py-1 px-1 text-right text-black">Amount</th>
                                                </tr>
                                            </thead>
                                            <tbody id="bankSlipsTableBody">
                                                <tr>
                                                    <td colspan="4"
                                                        class="py-2 px-1 text-center text-black text-xs italic">
                                                        No bank slip entries
                                                    </td>
                                                </tr>
                                            </tbody>
                                            <tfoot class="border-t-2 border-gray-300">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                                ...
                                <div class="bg-white rounded-lg border border-orange-300 overflow-hidden">
                                    ...
                                    <div class="p-0">
                                        ...
                                        <input type="hidden" id="credit_sales_amount" name="credit_sales_amount"
                                            value="0.00" />
                                        <input type="hidden" id="credit_sales" name="credit_sales"
                                            value='@json($creditSalesDecoded)' />
                                        ...
                                        <input type="hidden" id="recoveries_entries" name="recoveries_entries"
                                            value='@json($recoveriesDecoded)' />
                                        <input type="hidden" id="total_bank_transfers" name="total_bank_transfers"
                                            value="0.00" />
                                        <input type="hidden" id="bank_transfers" name="bank_transfers"
                                            value='@json($bankTransfersDecoded)' />
                                        <input type="hidden" id="total_bank_slips" name="total_bank_slips"
                                            value="{{ $settlement->bankSlips->sum('amount') }}" />
                                        <input type="hidden" id="bank_slips" name="bank_slips"
                                            value='@json($bankSlipsDecoded)' />
                                        <input type="hidden" id="total_cheques" name="total_cheques" value="0.00" />
                                        <input type="hidden" id="cheques" name="cheques"
                                            value='@json($chequesDecoded)' />
                                    </div>
                                </div>
                                                ...
                                                <template x-for="(expense, index) in expenses" :key="expense.id">
                                                    <tr class="hover:bg-gray-50">
                                                        <td class="py-1 px-1 text-xs">
                                                            ...
                                                            <template
                                                                x-if="expense.is_predefined && expense.account_code === '5223'">
                                                                <button type="button"
                                                                    class="text-xs font-semibold text-indigo-600 hover:text-indigo-700 underline"
                                                            ...
                                                            <template x-if="!expense.is_predefined">
                                                                <div class="flex items-center gap-0.5">
                                                                    <select @change="onAccountSelect($event, index)"
                                                                        ...
                                                                        <template
                                                                            x-for="account in getFilteredAccounts(expense.expense_account_id)"
                                                                            :key="account.id">
                                                        ...
                                                        <td class="py-1 px-1 text-right">
                                                            <input type="number" :id="'expense_amount_' + expense.id"
                                                                ...
                                                                :readonly="['1161', '5252', '5262', '5223'].includes(expense.account_code)"
                                                                :class="['1161', '5252', '5262', '5223'].includes(expense.account_code) ? 'bg-gray-100 cursor-not-allowed' : ''"
                                ...
                                <div class="bg-white rounded-lg border border-orange-300 overflow-hidden">
                                    ...
                                    <div id="payment_breakdown_warning"
                                        class="px-3 py-2 bg-red-100 border-b border-red-300 text-red-700 text-xs font-semibold"
                                    ...
                                    <div class="p-0">
                                        <table class="w-full text-xs">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="border-t border-gray-200">
                                                    <td class="py-1 px-1 text-xs text-black">Bank Slips / Deposits</td>
                                                    <td class="py-1 px-1 text-right font-semibold text-xs text-purple-700"
                                                        id="summary_bank_slips_display">0.00</td>
                                                </tr>
                                                <tr class="border-t border-gray-200">
                                                ...
                                                <tr class="bg-indigo-50 border-y-2 border-indigo-200">
                                                    <td class="py-1 px-1 text-xs font-semibold text-indigo-900">Expected
                                                        Cash (After Expenses)</td>
                                                ...
                                                <tr class="border-t border-gray-200">
                                                    <td class="py-1 px-1 text-xs text-black">
                                                        ...
                                                        <div class="text-[10px] text-gray-600 italic">
                                                            Total physical cash handed over to office
                                                        </div>
                                                    </td>
                                                    <td class="py-1 px-1 text-right font-semibold text-xs text-emerald-700"
        ...
        'tax_amount' => (float) $tax->tax_amount,
                                        ...
                                        <x-amr-expense-modal :products="$powderProducts" title="AMR Powder"
                                            ...
                                            :useBatchExpiry="$useBatchExpiry"
                                            :initialEntries="$settlement->amrPowders->map(fn($p) => [
        ...
        'unit_price' => $p->stock_batch_id ? ((float) ($p->stockBatch?->selling_price ?? 0)) : ((float) ($p->product?->expiry_price ?? 0)),
    ])" />
                                        <x-amr-expense-modal :products="$liquidProducts" title="AMR Liquid"
                                            ...
                                            :useBatchExpiry="$useBatchExpiry"
                                            :initialEntries="$settlement->amrLiquids->map(fn($l) => [
        ...
        'unit_price' => $l->stock_batch_id ? ((float) ($l->stockBatch?->selling_price ?? 0)) : ((float) ($l->product?->expiry_price ?? 0)),
    ])" />
                                        <x-bank-transfer-modal :customers="$customers" :bankAccounts="$bankAccounts"
                                            entriesInputId="bank_transfers" />
                                        <x-bank-slips-modal :bankAccounts="$bankAccounts" entriesInputId="bank_slips"
                                        ...
                                        <x-credit-sales-modal :customers="$customers" entriesInputId="credit_sales"
                                            :initialEntries="$creditSalesDecoded" />

                                        <x-recoveries-modal :customers="$customers" :bankAccounts="$bankAccounts"
                                            entriesInputId="recoveries_entries" />
                                        <x-percentage-expense-modal
                                            :customers="\App\Models\Customer::orderBy('customer_name')->get(['id', 'customer_name', 'customer_code'])" entriesInputId="percentage_expenses"
                                            triggerEvent="open-percentage-expense-modal"
                                            updatedEvent="percentage-expense-updated"
                                            :initialEntries="$settlement->percentageExpenses->map(fn($p) => [
        'customer_id' => $p->customer_id,
        'customer_name' => $p->customer ? $p->customer->customer_name . ' (' . $p->customer->customer_code . ')' : 'Unknown',
    ...
    @push('scripts')
        <script>
            ...
            const savedAmrPowders = @json($amrPowdersDecoded);
            const savedAmrLiquids = @json($amrLiquidsDecoded);
            const savedPercentageExpenses = @json($percentageExpensesDecoded);
            ...
            function expenseManager(expenseAccounts) {
                return {
                    ...
                    // Predefined expense accounts shown by default (with dynamically resolved IDs)
                    ...
                    init() {
                        ...
                        if (Array.isArray(savedExpenses) && savedExpenses.length) {
                            savedExpenses.forEach((expense) => {
                                const predefined = this.expenses.find(
                                    (item) => item.account_code === expense.account_code
                                );
                        ...
                        window.addEventListener('amr-powder-updated', (e) => {
                            const amrPowderExpense = this.expenses.find(exp => exp.account_code === '5252');
                            if (amrPowderExpense) {
                        ...
                        // Listen for AMR Liquid updates
                        window.addEventListener('amr-liquid-updated', (e) => {
                            const amrLiquidExpense = this.expenses.find(exp => exp.account_code === '5262');
                            if (amrLiquidExpense) {
                        ...
                        // Listen for Percentage Expense updates
                        window.addEventListener('percentage-expense-updated', (e) => {
                            const percentageExpense = this.expenses.find(exp => exp.account_code === '5223');
                            if (percentageExpense) {
                                percentageExpense.amount = e.detail.total || 0;
                                this.calculateTotal();
                            }
                        });
                    },
                    ...
                    getFilteredAccounts(currentAccountId) {
                        // Get list of currently selected account IDs (excluding the current row's account)
                        const selectedAccountIds = this.expenses
                            .map(exp => exp.expense_account_id)
                            .filter(id => id && id != currentAccountId);

                        // Filter available accounts to exclude those already selected
                        return this.availableAccounts.filter(acc => !selectedAccountIds.includes(acc.id));
                    },
            ...
            function preloadFinancialEntries() {
                ...
                setHiddenJson('amr_powders', savedAmrPowders);
                setHiddenJson('amr_liquids', savedAmrLiquids);
                setHiddenJson('percentage_expenses', savedPercentageExpenses);
            ...
            $(document).ready(function () {
                ...
                @elseif(isset($settlement))
                    const settlementGoodsIssue = @js([
                        'id' => $settlement->goods_issue_id,
                        'text' => $settlement->goodsIssue->issue_number . ' - ' . ($settlement->goodsIssue->employee->full_name ?? 'Unknown') . ' (' . $settlement->goodsIssue->issue_date->format('d M Y') . ')',
                    ]);

                    const option = new Option(settlementGoodsIssue.text, settlementGoodsIssue.id, true, true);
                    $('#goods_issue_id').append(option).trigger('change');

                    $('#goods_issue_id').trigger({
                        type: 'select2:select',
                        params: {
                            data: settlementGoodsIssue
                        }
                    });
                @endif
            ...
            function calculateBatchBalance(itemIndex, batchIndex, changedField = null) {
                ...
                if (totalUsed > totalAvailable) {
                    ...
                    // When user edits returned/shortage, keep that value and auto-reduce sold first.
                    if (lastField === 'returned' || lastField === 'shortage') {
                        sold = Math.max(0, totalAvailable - returned - shortage);
                        soldInput.value = sold;

                        // If returned + shortage alone exceed available, clamp the last edited field.
                        if (sold + returned + shortage > totalAvailable) {
                            shouldShowWarning = true;

                            if (lastField === 'returned') {
                                returned = Math.max(0, totalAvailable - shortage);
                                returnedInput.value = returned;
                            } else {
                                shortage = Math.max(0, totalAvailable - returned);
                                shortageInput.value = shortage;
                            }
                        }
                    } else {
                        // User changed sold, so clamp sold against current returned + shortage.
                        sold = Math.max(0, totalAvailable - returned - shortage);
                        soldInput.value = sold;
                        shouldShowWarning = true;
                    }
            ...
            function updateSalesSummary() {
                ...
                const bankSlipsAmount = parseFloat(document.getElementById('total_bank_slips').value) || 0;
                ...
                // Physical collection from payment channels (for reference display)
                ...
                // Cheques are cash-equivalent instruments submitted by the salesman (like bank slips).
                ...
                // Display total collections: net sale plus all recoveries
                ...
                const totalDeposited = actualPhysicalCash + chequeSalesAmount + bankSlipsAmount;

                // Shortage/Excess (Physical Cash + Cheques + Bank Slips vs Expected Cash)
                ...
                if (cashDisplay) {
                    cashDisplay.textContent = formatPKR(theoreticalCashSales);
                    if (theoreticalCashSales < 0) {
                        cashDisplay.classList.remove('text-emerald-700');
                        cashDisplay.classList.add('text-red-600', 'font-bold');
                    } else {
                        cashDisplay.classList.remove('text-red-600', 'font-bold');
                        cashDisplay.classList.add('text-emerald-700');
                    }
                }
                ...
                if (paymentWarning) {
                    if (theoreticalCashSales < 0) {
                        ...
                        paymentWarning.textContent = 'Credit sales and bank transfers exceed Net Sale (Sold Items Value) by Rs ' + excess + '. Please correct the payment entries.';
                    ...
                    updateDisplay() {
                        ...
                        if (this.entries.length === 0) {
                            ...
                            this.entries.forEach((entry, index) => {
                                ...
                                const newBalance = parseFloat(entry.new_balance) || 0;
                                ...
                                row.innerHTML = `
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        <td class="py-1 px-2 text-right text-xs font-semibold text-gray-600">
                                                                                                                                                                                                                                                            ₨ ${parseFloat(entry.previous_balance || 0).toLocaleString('en-PK', { minimumFractionDigits: 2 })}
                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                        <td class="py-1 px-2 text-right text-xs font-semibold text-orange-700">
                                                                                                                                                                                                                                                            ₨ ${saleAmount.toLocaleString('en-PK', { minimumFractionDigits: 2 })}
                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                        <td class="py-1 px-2 text-right text-xs font-semibold text-blue-700">
                                                                                                                                                                                                                                                            ₨ ${newBalance.toLocaleString('en-PK', { minimumFractionDigits: 2 })}
                                                                                                                                                                                                                                                        </td>
                                                                                                                                                                                                                                                    `;
                        ...
                        if (creditDisplay) {
                            creditDisplay.textContent = '₨ ' + totalSale.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }

                        const balanceDisplay = document.getElementById('creditBalanceTotalDisplay');
                        if (balanceDisplay) {
                            balanceDisplay.textContent = '₨ ' + totalBalance.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
            ...
            function recoveriesDisplay() {
                return {
                    ...
                    updateDisplay() {
                        ...
                        if (recoveryDisplay) {
                            recoveryDisplay.textContent = '₨ ' + totalRecovery.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
            ...
            function bankTransferDisplay() {
                return {
                    ...
                    updateDisplay() {
                        ...
                        const totalDisplay = document.getElementById('bankTransferTotalDisplay');
                        if (totalDisplay) {
                            totalDisplay.textContent = '₨ ' + totalBankTransfer.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
            ...
            function chequePaymentDisplay() {
                return {
                    ...
                    updateDisplay() {
                        ...
                        const totalDisplay = document.getElementById('chequeTotalDisplay');
                        if (totalDisplay) {
                            totalDisplay.textContent = '₨ ' + totalCheque.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
            ...
            function bankSlipsDisplay() {
                return {
                    ...
                    updateDisplay() {
                        ...
                        const totalDisplay = document.getElementById('bankSlipsTotalDisplay');
                        if (totalDisplay) {
                            totalDisplay.textContent = '₨ ' + totalBankSlips.toLocaleString('en-PK', { minimumFractionDigits: 2 });
                        }
                    },
