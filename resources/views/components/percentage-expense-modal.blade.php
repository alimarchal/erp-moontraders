@props([
    ...
    'inputId' => 'expense_percentage',
    'entriesInputId' => 'percentage_expense_entries',
    'initialEntries' => [],
])

<div x-data="percentageExpenseModal({
    ...
    <div x-show="show" class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6 sm:px-0">
        ...
        <div class="relative w-full max-w-5xl bg-white rounded-lg shadow-xl overflow-hidden transform transition-all flex flex-col max-h-[90vh]"
            ...
            <div
                ...
                <div>
                    <h3 class="text-lg font-semibold text-white">Percentage Expense (5223)</h3>
                    <p class="text-xs text-orange-100">Add per-customer percentage expense breakdown.</p>
                </div>
            ...
            <div class="p-6 space-y-4 flex-grow overflow-y-auto">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select id="percentage_expense_customer_select"
                ...
                <div class="flex justify-end">
                    <button type="button" @click="addEntry()"
                        class="px-6 py-2 bg-orange-600 text-white text-sm font-semibold rounded-md hover:bg-orange-700 shadow-sm">
                        Add Entry
                    </button>
                </div>
                ...
                <div class="overflow-x-auto border border-gray-200 rounded-lg">
                    <table class="min-w-full text-sm">
                            ...
                            <template x-if="entries.length === 0">
                                <tr>
                                    <td colspan="5" class="px-3 py-4 text-center text-gray-500 italic">
                                        No percentage expense entries added yet.
                                    </td>
                                </tr>
                            </template>
                        ...
                        <tfoot class="bg-orange-50 border-t-2 border-orange-200">
                            <tr>
                                <td colspan="3" class="px-3 py-2 text-right font-bold text-orange-900">Grand Total</td>
...
@once
    @push('scripts')
        <script>
            function percentageExpenseModal({ customers, inputId, entriesInputId, initialEntries }) {
                return {
                    show: false,
                    customers,
                    form: {
                    ...
                    initializeSelect2() {
                        const self = this;
                        $('#percentage_expense_customer_select').select2({
                            width: '100%',
                            placeholder: 'Select Customer',
                            allowClear: true,
                            dropdownParent: $('#percentage_expense_customer_select').parent()
                        });

                        // Handle select2 change event
                        $('#percentage_expense_customer_select').on('change', function() {
                    ...
                    addEntry() {
                        ...
                        $('#percentage_expense_customer_select').val(null).trigger('change');
                    },
                    ...
                    syncTotals() {
                        ...
                        window.dispatchEvent(new CustomEvent('percentage-expense-updated', {
                            detail: { total: total }
                        }));
                    },
